...
function findLinePolygonIntersections(start, end, polygon) {
  ...
  for (const entry of intersections) {
    ...
    if (last) {
      const deltaT = Math.abs(entry.t - last.t);
      const deltaX = entry.point.x - last.point.x;
      const deltaY = entry.point.y - last.point.y;
      if (deltaT <= TOL && Math.hypot(deltaX, deltaY) <= 1e-6) {
        continue;
      }
    }
    deduped.push(entry);
...
function linesInPolygon(polygonPoints, spacing, angleDeg, offset = 0) {
  ...
  for (let i = -numLines; i <= numLines; i += 1) {
    ...
    for (let j = 0; j < intersections.length - 1; j += 1) {
      ...
      const mid = { x: p1.x + dx * 0.5, y: p1.y + dy * 0.5 };
